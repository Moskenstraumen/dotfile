...
# The external display is the main display; the built-in display is the extension
MAIN=1
BUILTIN=2
...
yabai -m space "$SPACE4" --display "$BUILTIN" 2>/dev/null || true
yabai -m space "$SPACE5" --display "$BUILTIN" 2>/dev/null || true
