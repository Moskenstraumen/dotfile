...
if command -v yabai >/dev/null 2>&1; then
  ...
  SPACE_EVENTS="front_app_switched space_change yabai_window_focus yabai_window_created yabai_window_destroyed yabai_window_minimized yabai_window_deminimized"
else
...
for sid in $workspaces; do
  ...
  if [ "$SBAR_BAR_STYLE" = "block" ]; then
    ...
    # Subscribe to each event separately
    for event in $SPACE_EVENTS; do
      sketchybar --subscribe "space.$sid" "$event"
    done
  else
    sketchybar --add space "space.$sid" left \
      --set "space.$sid" \
