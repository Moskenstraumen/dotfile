...
update() {
  ...
  if [ "$STATE" = "playing" ]; then
    TRACK=$(osascript -e 'tell application "Spotify" to name of current track' | cut -c1-24)
  fi

  args=()

  if [ "$STATE" = "playing" ]; then
    args+=(--set spotify.name label="${TRACK}" label.drawing=on drawing=on)
    args+=(--set spotify.play icon=󰏤)
  else
    args+=(--set spotify.name label="" label.drawing=off drawing=on)
